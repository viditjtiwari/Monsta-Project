...
export default function ViewLocation() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_COUNTRY_URL}view`, {
      page: currentPage,
      limit: 10,
      name: searchName,
    })
  ...
  const allCheckBox = () => {
    if (checkbox.length == country.length) {
      setCheckbox([])
      setBtnDisabled(true)
    } else {
      setCheckbox([]);

      const finalData = country.map((v) => {
        return v._id;
      });
  ...
  const changeStatus = () => {
    axios.put(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_COUNTRY_URL}change-status`, {
      ids: checkbox
    })
      .then((result) => {
        if (result.data.status == true) {
  ...
  const deleteRecord = () => {
    axios.put(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_COUNTRY_URL}delete`, {
      ids: checkbox
    })
      .then((result) => {
        if (result.data.status == true) {
  ...
  return (
    <section className="w-full">
      ...
      <div className={` rounded-lg border border-gray-300 px-5 py-5 max-w-[1220px] mx-auto mt-10 ${activeFilter ? "hidden" : "block"}`}>
        <form onSubmit={searchHandler} className="flex max-w-sm">
                    ...
                    <tr>
                      <th scope="col" className="p-4">
                        <div className="flex items-center">
                          <input
                            onClick={allCheckBox}
                            checked={checkbox.length == country.length ? 'checked' : ""}
                            readOnly
