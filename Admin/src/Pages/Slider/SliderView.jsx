...
export default function SliderView() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_SLIDER_URL}view`, {
      page: currentPage,
      limit: 10,
      name: searchName,
    })
  ...
  const allCheckBox = () => {
    if (checkbox.length == slider.length) {
      setCheckbox([])
      setBtnDisabled(true)
    } else {
      setCheckbox([]);

      const finalData = slider.map((v) => {
        return v._id;
      });
  ...
  const changeStatus = () => {
    axios.put(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_SLIDER_URL}change-status`, {
      ids: checkbox
    })
      .then((result) => {
        if (result.data.status == true) {
  ...
  const deleteRecord = () => {
    axios.put(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_SLIDER_URL}delete`, {
      ids: checkbox
    })
      .then((result) => {
        if (result.data.status == true) {
                    ...
                    <tr>
                      <th scope="col" className="p-4">
                        <div className="flex items-center">
                          <input
                            onClick={allCheckBox}
                            checked={checkbox.length == slider.length ? 'checked' : ""}
                            readOnly
