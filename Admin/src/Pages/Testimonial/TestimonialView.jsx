...
export default function TestimonialView() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_TESTIMONIAL_URL}view`, {
      page: currentPage,
      limit: 10,
      name: searchName,
    })
  ...
  const allCheckBox = () => {
    if (checkbox.length == testimonial.length) {
      setCheckbox([])
      setBtnDisabled(true)
    } else {
      setCheckbox([]);

      const finalData = testimonial.map((v) => {
        return v._id;
      });
  ...
  const changeStatus = () => {
    axios.put(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_TESTIMONIAL_URL}change-status`, {
      ids: checkbox
    })
      .then((result) => {
        if (result.data.status == true) {
  ...
  const deleteRecord = () => {
    axios.put(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_TESTIMONIAL_URL}delete`, {
      ids: checkbox
    })
      .then((result) => {
        if (result.data.status == true) {
                      ...
                      <th scope="col" className="p-4">
                        <div className="flex items-center">
                          <input
                            ...
                            onClick={allCheckBox}
                            checked={checkbox.length == testimonial.length ? 'checked' : ""}
                            readOnly
                            type="checkbox"
...

                    {
                      testimonial.length > 0
                        ...
                        <tr className="bg-white border-gray-200 hover:bg-gray-50">
                          <th colSpan={8} className="text-center px-6 py-4 text-gray-900 whitespace-nowrap">
                            <div className="py-4">
