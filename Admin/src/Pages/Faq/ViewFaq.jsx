...
export default function ViewFaq() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_FAQ_URL}view`, {
      page: currentPage,
      limit: 10,
      name: searchName,
      code: searchCode,
  ...
  const allCheckBox = () => {
    if (checkbox.length == faq.length) {
      setCheckbox([])
      setBtnDisabled(true)
    } else {
      setCheckbox([]);

      const finalData = faq.map((v) => {
        return v._id;
      });
  ...
  const changeStatus = () => {
    axios.put(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_FAQ_URL}change-status`, {
      ids: checkbox
    })
      .then((result) => {
        if (result.data.status == true) {
  ...
  const deleteRecord = () => {
    axios.put(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_FAQ_URL}delete`, {
      ids: checkbox
    })
      .then((result) => {
        if (result.data.status == true) {
                    ...
                    <tr>
                      <th scope="col" className="p-4">
                        <div className="flex items-center">
                          <input
                            onClick={allCheckBox}
                            checked={checkbox.length == faq.length ? 'checked' : ""}
                            readOnly
