...
exports.view = async (request, response) => {
    ...
    try {
        ...
        await defaultModal.find(filter).select("name image status order").skip(skip).limit(limit).sort({ _id: "desc" }) //For Website this would be .sort({order: "asc", _id: "desc"})
            .then((result) => {
                if (result.length > 0) {
                    var paginate = {
                    ...
                    const data = {
                        ...
                        paginate: paginate,
                        image_path: process.env.default_image_path,
                        data: result
                    }
                    response.send(data);
                }
