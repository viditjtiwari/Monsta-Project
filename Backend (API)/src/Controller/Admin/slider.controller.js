...
exports.create = async (request, response) => {
    ...
    try {
        ...
        var saveData = new sliderModal(data).save()
            .then((result) => {
                const data = {
                    status: true,
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        total_records = await sliderModal.find(filter).countDocuments()

        await sliderModal.find(filter).select("name image status order").skip(skip).limit(limit).sort({ _id: "desc" }) //For Website this would be .sort({order: "asc", _id: "desc"})
            .then((result) => {
                if (result.length > 0) {
                    var paginate = {
                    ...
                    const data = {
                        ...
                        paginate: paginate,
                        image_path: process.env.slider_image_path,
                        data: result
                    }
                    response.send(data);
                }
...
exports.details = async (request, response) => {
    try {
        ...
        await sliderModal.findById(request.params.id)
            .then((result) => {
                if (result) {
...
exports.update = async (request, response) => {
    try {
        ...
        var saveData = await sliderModal.updateOne({
...
exports.destroy = async (request, response) => {
    try {
        ...
        var saveData = await sliderModal.updateMany({
