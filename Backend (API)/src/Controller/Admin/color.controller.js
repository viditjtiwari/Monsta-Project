...
exports.create = (request, response) => {
    ...
    //Parameter till now {name : required, code, order}
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        await colorModal.find(filter).select("name code status order").skip(skip).limit(limit).sort({ _id: "desc" }) //For Website this would be .sort({order: "asc", _id: "desc"})
            .then((result) => {
                if (result.length > 0) {
                    var paginate = {
